...
module.exports = class commandErrorEvent extends Event {
	...
	async run(msg, command, params, error) {
		...
		if (error.message) return msg.sendMessage(`There has been an error while running the command, Try again and if the problem incurs again contact ${this.client.owner.tag}\n${codeBlock('json', error.message)}`).catch(err => this.client.emit('wtf', err));
