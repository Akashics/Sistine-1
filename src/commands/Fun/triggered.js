...
module.exports = class Crush extends Command {
	...
	async run(msg, [user = msg.author]) {
		const message = await msg.send(msg.language.get('COMMAND_TRIGGERING', user.tag));
		const attachment = await this.getTriggered(user.displayAvatarURL({ format: 'png', size: 512 }));
		await msg.send({ files: [{ attachment, name: 'triggered.gif' }] });
	...
	async getTriggered(triggered) {
		...
		imgTitle.src = await readFile('./src/assets/images/plate_triggered.png');
