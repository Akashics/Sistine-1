...
module.exports = class Reputation extends Command {
	...
	async run(msg, [user = msg.author]) {
		...
		if (Date.now() > giver.conf.reputationTimer) {
			const message = await msg.send(msg.language.get('COMMAND_REPUTATION_SENT', user));
			await giver.conf.update('reputationTimer', message.createdTimestamp + (24 * 60 * 60 * 1000));
			return reciever.conf.update('reputation', reciever.conf.reputation + 1);
		} else {
