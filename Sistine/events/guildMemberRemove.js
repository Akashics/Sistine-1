...
module.exports = class guildMemberRemove extends Event {
	...
	async run(member) {
		...
		const { memberChannel, playerLogLevel, leaveMessages } = member.guild.configs.logging;
		if (!memberChannel || playerLogLevel <= 0 || !leaveMessages) { return; }

		this.client.stats.increment('client.memberLeaves');
		if (!member.guild.configs.logging.memberChannel || member.guild.configs.logging.playerLogLevel === 0) { return; }
		const byebyemsg = [
