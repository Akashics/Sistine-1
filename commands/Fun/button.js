...
Canvas.registerFont('./assets/fonts/ComicSansMS2.ttf', { family: 'ComicSans' });
...
module.exports = class Button extends Command {
	...
	async run(msg, [text]) {
		const button = await fsn.readFile('./assets/images/button.png');
		const attachment = new Canvas(504, 375)
			...
			.toBuffer();
		return msg.channel.send({ files: [{ attachment, name: 'button.png' }] });
	}
