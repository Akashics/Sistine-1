...
module.exports = class extends Command {
	...
	async run(msg, [user]) {
		...
		if (!blocklist.includes(user.id)) {
			return msg.send(`<:tickNo:315009174163685377> \`${user.tag}[${user.id}]\` is not blocklisted.`);
		}
		await blocklist.splice(blocklist.indexOf(user.id));
		await writeJSONAtomic('./keys/blocklist.json', this.client.blocklist);
		return msg.send(`<:tickYes:315009125694177281> \`${user.tag}[${user.id}]\` was removed from the blocklist.`);
	}
