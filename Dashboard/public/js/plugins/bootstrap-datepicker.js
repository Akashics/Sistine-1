...
(function (factory) {
	...
	var DateArray = (function () {
		var extras = {
			...
			contains: function (d) {
				...
				{
					if (this[i].valueOf() - val >= 0 && this[i].valueOf() - val < 1000 * 60 * 60 * 24) { return i; }
				}
				return -1;
			},
			remove: function (i) {
	...
	var Datepicker = function (element, options) {
		...
		if (this.o.calendarWeeks) {
			this.picker.find('.datepicker-days .datepicker-switch, thead .datepicker-title, tfoot .today, tfoot .clear')
				.attr('colspan', (i, val) => Number(val) + 1);
		}
	...
	Datepicker.prototype = {
		...
		_resolveViewName: function (view) {
			$.each(DPGlobal.viewModes, (i, viewMode) => {
		...
		_process_options: function (opts) {
			...
			o.datesDisabled = $.map(o.datesDisabled, (d) => DPGlobal.parseDate(d, format, o.language, o.assumeNearbyYear));

			var plc = String(o.orientation).toLowerCase().split(/\s+/g),
				_plc = o.orientation.toLowerCase();
			plc = $.grep(plc, (word) => /^auto|left|right|top|bottom$/.test(word));
			o.orientation = {
				x: 'auto',
				y: 'auto'
			};
			if (!_plc || _plc === 'auto') { } // no action
				...
				_plc = $.grep(plc, (word) => /^left|right$/.test(word));
				o.orientation.x = _plc[0] || 'auto';

				_plc = $.grep(plc, (word) => /^top|bottom$/.test(word));
		...
		getUTCDates: function () {
			return $.map(this.dates, (d) => new Date(d));
		},
		...
		getFormattedDate: function (format) {
			...
			return $.map(this.dates, (d) => DPGlobal.formatDate(d, format, lang)).join(this.o.multidateSeparator);
		},
		...
		place: function () {
			...
			var zIndex = Math.max(...parentsZindex) + this.o.zIndexOffset;
		...
		setRange: function (range) {
			if (!range || !range.length) { delete this.range; } else {
				this.range = $.map(range, (d) => d.valueOf());
			}
			this.fill();
		},
		...
		_fill_yearsView: function (selector, cssClass, factor, year, startYear, endYear, beforeFn) {
			...
			var selected = $.map(this.dates, (d) => Math.floor(d.getUTCFullYear() / step) * step);
		...
		dateIsDisabled: function (date) {
			return (
				this.weekOfDateIsDisabled(date) ||
																$.grep(this.o.datesDisabled, (d) => isUTCEquals(date, d)).length > 0
			);
		},
	...
	var DateRangePicker = function (element, options) {
		$.data(element, 'datepicker', this);
		this.element = $(element);
		this.inputs = $.map(options.inputs, (i) => i.jquery ? i[0] : i);
		...
		this.pickers = $.map(this.inputs, (i) => $.data(i, 'datepicker'));
		this.updateDates();
	};
	DateRangePicker.prototype = {
		updateDates: function () {
			this.dates = $.map(this.pickers, (i) => i.getUTCDate());
			this.updateRanges();
		},
		updateRanges: function () {
			var range = $.map(this.dates, (d) => d.valueOf());
			$.each(this.pickers, (i, p) => {
				p.setRange(range);
			});
		},
	...
	var DPGlobal = {
		...
		parseDate: function (date, format, language, assumeNearby) {
			...
			if (parts.length !== fparts.length) {
				fparts = $(fparts).filter((i, p) => $.inArray(p, setters_order) !== -1).toArray();
			}
			// Process remainder
			function match_part() {
